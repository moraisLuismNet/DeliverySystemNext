...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const items = await menuItemService.getAvailableByRestaurantIdAsync(parseInt(id));
    return ResponseHelper.success("Available menu items retrieved successfully", items);
  } catch (error: any) {
